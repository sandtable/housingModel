...
import housing.Collectors;
import housing.MortgageApproval;
...
public class Bank extends EconAgent {
	 ...
	 */
	public void step() {
		supplyTarget = 1000.0 * Model.root.setOfHouseholds.size();
	 ...
	 ****************************/
	public MortgageApproval requestLoan(Mortgage.Borrower h, double housePrice, double desiredDownPayment, boolean isHome) {
...
//		Collectors.creditSupply.recordLoan(h, approval);
		++nLoans;
		if(isHome) {
			if(approval.principal/(h.monthlyIncome()*12.0) > centralBank.loanToIncomeRegulation(h,isHome)) {
				++nOverLTICapLoans;
			}
	 ...
	 */
	public MortgageApproval requestApproval(Mortgage.Borrower h, double housePrice, double desiredDownPayment, boolean isHome) {
		...
		approval.principal = Math.max(0.0,h.monthlyDisposableIncome())/monthlyPaymentFactor();

		ltv_principal = housePrice*loanToValue(h, isHome);
		approval.principal = Math.min(approval.principal, ltv_principal);

		lti_principal = h.monthlyIncome()*12.0 * loanToIncome(h,isHome);
