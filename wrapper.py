...
class WrappedModel(ShellSimulator):
    ...
    def __init__(self, config):
        ...
        # XXX hack: change to input data directory
        # to do with location of input data
        input_folder = os.path.dirname(self.inputs.values()[0])
        logger.info('Change cwd from: {} to: {}'
                    .format(os.getcwd(), input_folder))
        os.chdir(input_folder)

        binary = os.path.join(self.binary_folder, self.binaries['jar'])
